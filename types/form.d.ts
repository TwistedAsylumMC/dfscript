...
declare namespace form {
    ...
    interface CustomForm extends Form {
        ...
        onSubmit(callback: (player: player.Player, data: any[], closed: boolean) => void): CustomForm;
    }
    function custom(title: string): CustomForm;

    interface MenuForm extends Form {
        button(text: string, image?: string, submit?: (player: player.Player) => void): MenuForm;
        onSubmit(callback: (player: player.Player, index: number, closed: boolean) => void): MenuForm;
    }
    function menu(title: string, body?: string): MenuForm;

    interface ModalForm extends Form {
        yes(text: string, submit?: (player: player.Player) => void): ModalForm;
        no(text: string, submit?: (player: player.Player) => void): ModalForm;
        onSubmit(callback: (player: player.Player, index: number, closed: boolean) => void): ModalForm;
    }
